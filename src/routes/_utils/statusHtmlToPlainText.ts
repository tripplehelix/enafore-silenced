...
export function statusDomToPlainText (doc: DefaultTreeAdapterMap['parentNode'], mentions?: Mention[]): string {
    ...
    for (const child of node.childNodes) {
      if (defaultTreeAdapter.isElementNode(child)) {
        if (child.tagName === 'br') {
          ...
          if ((mentions != null) && (c != null) && c.value.split(/\s+/).includes('mention') && (h != null) && ((m = mentions.find((mention) => h.value === mention.url || h.value === '/accounts/' + mention.id)) != null)) {
            s += '@' + m.acct
            continue
          }
