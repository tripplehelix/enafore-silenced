...
export function emojifyText (text, emojis, autoplayGifs) {
  ...
  text = replaceEmoji(text, substring => {
    if(testEmojiSupported(substring)) {
      ...
      return `<span class="inline-emoji unsupported" style="background-image:url('https://github.com/googlefonts/noto-emoji/blob/main/png/72/emoji_u${[...substring].map(e=>e.codePointAt(0).toString(16))}.png?raw=tru')">${substring}</span>`
    }
  })
