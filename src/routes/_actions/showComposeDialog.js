...
export async function showComposeDialog () {
  ...
  const [showComposeDialog, twitterPost] = await Promise.all([importShowComposeDialogPromise, Promise.resolve(new URLSearchParams(location.search).get('retweet')).then(rt=>rt&&fetch(Object.assign(new URL(rt, {hostname: "birdlink.easrng.workers.dev"})).href)).then(e=>e.text).catch(e=>null)])
